// BTC 15m Quant Relay — V11.2
// DepthFeed + Kalshi only. Robinhood execution is intentionally manual in the dashboard.
...
const mem = {
  dfLastGood: null, dfLastGoodMs: 0, dfBackoffUntil: 0,
  kalshiBackoffUntil: 0, kalshiLastGoodMarket: null,
  kalshiLastGoodOrderbook: new Map(), kalshiSettledCache: null, kalshiSettledCacheMs: 0,
};
...
async function depthfeedGet(env, path) {
  ...
  if (r.status === 429) {
    mem.dfBackoffUntil =
      Date.now() + retryMs(r.headers, 3000);
  }
